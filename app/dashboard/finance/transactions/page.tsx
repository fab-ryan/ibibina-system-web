...
function exportCSV(rows: Transaction[]) {
    ...
    const lines = rows.map((t) => [
        fmtDate(t.transactionDate), memberName(t), TYPE_LABELS[t.type] ?? t.type,
        t.direction, String(t.amount), t.currency, t.paymentMethod ?? "",
        t.momoRef ?? t.bankRef ?? "", t.approvalStatus ?? "", t.description ?? "",
...
export default function FinanceTransactionsPage() {
    ...
    const pendingCash = useMemo(() => filtered.filter(needsApproval).length, [filtered]);
        ...
        <>
            <div className="grid gap-6">
                ...
                <section className="flex flex-wrap gap-3">
                    ...
                    >
                        {ALL_TYPES.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                    </select>
                </section>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-[860px] text-left text-sm">
                            ...
                            <tbody className="divide-y divide-(--ib-line)">
                                {isLoading ? (
                                    <tr><td colSpan={8} className="px-5 py-10 text-center text-sm text-(--ib-muted)">Loading transactions…</td></tr>
                                ) : filtered.length === 0 ? (
                                    <tr><td colSpan={8} className="px-5 py-10 text-center text-sm text-(--ib-muted)">No transactions found.</td></tr>
                                ) : (
                                    filtered.map((tx) => (
                                        <TransactionRow
                                            key={tx.id}
            ...
            {/* View Details Modal */}
            {selected && (
                <Modal isOpen={activeModal === "view"} onClose={close} title="Transaction Details" description={`${TYPE_LABELS[selected.type] ?? selected.type} — ${fmtDate(selected.transactionDate)}`}>
                    <div className="grid gap-4 p-5">
                        {isCash(selected) && (
                            <div className={`flex items-start gap-3 rounded-lg border p-3 ${selected.approvalStatus === "approved" ? "border-green-200 bg-green-50"
                                    : selected.approvalStatus === "rejected" ? "border-red-200 bg-red-50"
                                ...
                                {needsApproval(selected) && (
                                    <div className="flex shrink-0 gap-2">
                                        <button onClick={() => { handleApprove(selected); close(); }} disabled={isApproving}
                                            ...
                                            {isApproving ? <Spinner size="sm" /> : <CheckCircle2 size={13} />} Approve
                                        </button>
                                        <button onClick={() => { close(); openReject(selected); }}
                        ...
                        {selected.referenceFileUrl && (
                            <div className="rounded-lg border border-(--ib-line) p-3">
                                ...
                                <a href={selected.referenceFileUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-(--ib-accent) underline break-all">
                                    {selected.referenceFileUrl}
                                </a>
                            </div>
                        )}
            ...
            {/* Reject Modal */}
            {selected && (
                <Modal isOpen={activeModal === "reject"} onClose={close} title="Reject Transaction" description={`Reject cash payment from ${memberName(selected)} — ${fmt(selected.amount, selected.currency)}`}>
                    <div className="grid gap-4 p-5">
                        <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-3">
                            <XCircle size={18} className="mt-0.5 shrink-0 text-red-600" />
                            <p className="text-xs text-red-700">Rejecting this transaction will flag it as invalid. Provide a reason so the member can be informed.</p>
                        </div>
                        <div>
                            <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-(--ib-muted)">Reason (optional)</label>
                            <textarea
                        ...
                        <div className="flex justify-end gap-3 border-t border-(--ib-line) pt-4">
                            ...
                            <button className="ib-btn-primary gap-2 bg-red-600 hover:bg-red-700" onClick={handleReject} disabled={isRejecting}>
...
function TransactionRow({ tx, isApproving, onView, onApprove, onReject }: {
    ...
    return (
        <tr className={`hover:bg-gray-50/60 ${pending ? "bg-amber-50/40" : ""}`}>
            ...
            <td className="px-5 py-3 font-semibold text-(--ib-ink)">{fmt(tx.amount, tx.currency)}</td>
            <td className="px-5 py-3">
                {tx.direction === "credit" ? (
                    <span className="inline-flex items-center gap-1 text-green-700"><ArrowDownLeft size={14} /> Credit</span>
                ) : (
                    <span className="inline-flex items-center gap-1 text-red-600"><ArrowUpRight size={14} /> Debit</span>
                )}
            </td>
            <td className="px-5 py-3 capitalize text-(--ib-muted)">
                {cash ? (
                    <span className="inline-flex items-center gap-1 font-medium text-amber-700"><Banknote size={13} /> Cash</span>
                ) : (tx.paymentMethod ?? "—")}
            </td>
            <td className="px-5 py-3">
                {cash ? (
                    <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${APPROVAL_BADGE[tx.approvalStatus ?? "pending_approval"]}`}>
                        {tx.approvalStatus === "approved" ? "Approved" : tx.approvalStatus === "rejected" ? "Rejected" : "Pending"}
                    </span>
                ...
                {menuOpen && menuRect && createPortal(
                    <>
                        ...
                        >
                            ...
                            {pending && (
                                <>
                                    <button onClick={() => { setMenuOpen(false); onApprove(); }} disabled={isApproving} className="flex w-full items-center gap-2.5 px-4 py-2 text-sm hover:bg-gray-50 text-green-700 disabled:opacity-60">
                                        <CheckCircle2 size={14} /> Approve Cash
                                    </button>
                                    <button onClick={() => { setMenuOpen(false); onReject(); }} className="flex w-full items-center gap-2.5 px-4 py-2 text-sm hover:bg-gray-50 text-red-600">
